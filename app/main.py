...
GITHUB_REF_API = "https://api.github.com/repos/{repo}/git/ref/heads/{branch}"
GITHUB_REFS_API = "https://api.github.com/repos/{repo}/git/refs"
...
def github_operation_config() -> dict[str, str]:
    return {
        ...
        "source_branch": os.getenv("GITHUB_OPERATION_SOURCE_BRANCH", "").strip(),
...
def github_ref_url(repo: str, branch: str) -> str:
    return GITHUB_REF_API.format(repo=repo, branch=urllib.parse.quote(branch, safe="/"))
...
def ensure_github_branch(config: dict[str, str], headers: dict[str, str]) -> dict[str, Any]:
    branch = config["branch"]
    repo = config["repo"]
    branch_response = requests.get(github_ref_url(repo, branch), headers=headers, timeout=20)
    if branch_response.status_code == 200:
        return {"ok": True, "created": False, "branch": branch}
    if branch_response.status_code != 404:
        return {"ok": False, "status_code": branch_response.status_code, "message": branch_response.text[:500]}

    repo_response = requests.get(GITHUB_REPO_API.format(repo=repo), headers=headers, timeout=20)
    if repo_response.status_code != 200:
        return {"ok": False, "status_code": repo_response.status_code, "message": repo_response.text[:500]}
    default_branch = repo_response.json().get("default_branch") or "main"
    source_branch = config.get("source_branch") or default_branch
    source_response = requests.get(github_ref_url(repo, source_branch), headers=headers, timeout=20)
    if source_response.status_code != 200 and source_branch != default_branch:
        source_branch = default_branch
        source_response = requests.get(github_ref_url(repo, source_branch), headers=headers, timeout=20)
    if source_response.status_code != 200:
        return {"ok": False, "status_code": source_response.status_code, "message": source_response.text[:500]}

    source_sha = source_response.json().get("object", {}).get("sha")
    if not source_sha:
        return {"ok": False, "message": f"无法读取源分支 {source_branch} 的 SHA"}
    create_response = requests.post(
        GITHUB_REFS_API.format(repo=repo),
        headers=headers,
        json={"ref": f"refs/heads/{branch}", "sha": source_sha},
        timeout=20,
    )
    if create_response.status_code in {201, 422}:
        return {"ok": True, "created": create_response.status_code == 201, "branch": branch, "source_branch": source_branch}
    return {"ok": False, "status_code": create_response.status_code, "message": create_response.text[:500]}
...
def github_put_file(repo_path: str, content: bytes, commit_message: str) -> dict[str, Any]:
    ...
    headers = github_api_headers(config["token"])
    try:
        branch_result = ensure_github_branch(config, headers)
        if not branch_result.get("ok"):
            return {"enabled": True, "ok": False, "repo_path": repo_path, "branch": config["branch"], "branch_result": branch_result}
        ...
        if existing.status_code == 200:
            ...
            return {"enabled": True, "ok": False, "status_code": existing.status_code, "repo_path": repo_path, "branch_result": branch_result, "message": existing.text[:500]}
        response = requests.put(url, headers=headers, json=payload, timeout=30)
        return {"enabled": True, "ok": 200 <= response.status_code < 300, "status_code": response.status_code, "repo_path": repo_path, "branch_result": branch_result, "message": response.text[:500]}

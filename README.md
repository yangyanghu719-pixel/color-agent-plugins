...
4. 图像区采用三栏：上传原图、当前选中色块区域、调整后整图实时预览；
5. 当前选中色块区域专用于帮助学生理解“正在调整哪里”，支持 hover 临时观察与 click 锁定观察；
6. 右栏预览是 Canvas 的整图实时预览，只对选区做 HSL 局部变化，不展示 mask 图本身；
7. H/S/L 滑杆采用颜色语义渐变，并实时显示原始值 → 当前值与 ΔH/ΔS/ΔL；
8. 点击“记录这次调整”后调用 `POST /recolor`，由后端生成正式预览图；
9. 点击“生成实验反馈”后调用 `POST /analyze`，以学习反馈卡片展示解释；

...
4. 支持选择色块，并在原图上高亮对应 mask 区域；
5. 支持 H/S/L 手动调节，展示当前值与 ΔH/ΔS/ΔL 变化值；
6. 支持前端 Canvas 实时预览局部调色效果（拖动滑杆时不频繁请求后端）；
7. 点击“记录这次调整”后调用 `POST /recolor`，由后端生成正式预览图；
8. 点击“生成实验反馈”后调用 `POST /analyze`，以学习反馈卡片展示解释；
9. 提供“实验导师”侧边栏 UI 占位，当前版本不接入 HiAgent；
10. 当前仍是 MVP 近似实验：基于颜色聚类与 mask，不是 Photoshop 级修图。
